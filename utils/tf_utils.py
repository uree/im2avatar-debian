...
def conv3d_transpose(inputs,
  ...
  """
  with tf.variable_scope(scope) as sc:
    kernel_h, kernel_w, kernel_d = kernel_size
    num_in_channels = inputs.get_shape()[-1]
    kernel_shape = [kernel_h, kernel_w, kernel_d,
                    num_output_channels, num_in_channels] # reversed to conv2d
    kernel = _variable_with_weight_decay('weights',
                                         shape=kernel_shape,
                                         use_xavier=use_xavier,
    ...
    output_shape = [batch_size, out_height, out_width, out_depth, num_output_channels]
...
def upsample_vol(vol, scales=[2,2,2]):
  """ vol: (batch_size, vol_dim, vol_dim, vol_dim, vol_chan)
      scales: (depth, height, width)

      Returns
        (batch_size, vol_dim * depth, vol_dim*height, vol_dim * width, 3)
  """
  batch_size = vol.get_shape()[0]
  vol_dim = vol.get_shape()[1]
  vol_chan = vol.get_shape()[4]
  repeat_depth = scales[0]
  repeat_height = scales[1]
  repeat_width = scales[2]

  vol_rep_width = tf.tile(tf.reshape(vol, (-1, vol_chan)), (1, repeat_width))
  vol_rep_width = tf.reshape(vol_rep_width, (batch_size, vol_dim, vol_dim, vol_dim * repeat_width, vol_chan))

  vol_rep_height = tf.tile(tf.reshape(vol_rep_width, (-1, vol_dim * repeat_width, vol_chan)), (1, repeat_height, 1))
  vol_rep_height = tf.reshape(vol_rep_height, (batch_size, vol_dim, vol_dim * repeat_height, vol_dim*repeat_width, vol_chan))

  vol_rep_depth = tf.tile(tf.reshape(vol_rep_height, (-1, vol_dim * repeat_height, vol_dim * repeat_width, vol_chan)), (1, repeat_depth, 1, 1))
  vol_rep_depth = tf.reshape(vol_rep_depth, (batch_size, vol_dim * repeat_depth, vol_dim * repeat_height, vol_dim*repeat_width, vol_chan))

  return vol_rep_depth


def Sampler(fields, images):
  """ Args:
  ...
  batch = fields.get_shape()[0]
  vol_dim = fields.get_shape()[1]
  im_height = images.get_shape()[1]
  im_width = images.get_shape()[2]
  fields_dim = fields.get_shape()[-1]
  im_dim = images.get_shape()[-1]

  im_flat = tf.reshape(images, [-1, im_dim])

  zero = tf.constant(0, dtype='int32')
  max_x = tf.cast(im_width - 1, 'int32')
  max_y = tf.cast(im_height - 1, 'int32')

  im_height_f = tf.cast(im_height, 'float32')
  im_width_f = tf.cast(im_width, 'float32')

  # fields
  x = fields[:, :, :, :, 0] # (batch, vol_dim, vol_dim, vol_dim)
  y = fields[:, :, :, :, 1] # (batch, vol_dim, vol_dim, vol_dim)

  # transform [0, 1] to [0, im_dim-1]
  x = x * (im_width_f)
  y = y * (im_height_f)
  ...
  base_idx = tf.range(batch) * im_height * im_width
  base_idx = tf.reshape(base_idx, [batch, 1, 1, 1])

  index += base_idx

  color = tf.gather(im_flat, index)
